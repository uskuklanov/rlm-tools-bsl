...
Плотный батчинг хелперов: снижение числа `rlm_execute` и пере-резолва объектов **без потери качества анализа** (главный эффект — устранение обрывов по лимиту вызовов в худшем случае и контрактного шума, а не средняя цена). Read-only слой поверх существующего индекса: схема и `BUILDER_VERSION` (14) НЕ менялись, пересборка боевых индексов НЕ требуется.
