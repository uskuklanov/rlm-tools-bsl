...
### Режим стратегии: `slim` (по умолчанию) vs `full`

С v1.11.0 `rlm_start` отдаёт **slim**-стратегию — компактную маршрутную карту по инструментарию песочницы, а детальные рецепты, описания пар и тексты секций агент дозагружает по запросу через новый MCP-tool `rlm_help(...)`. Это экономит токены контекста на старте сессии: на больших проектах full-стратегия — ~20К символов inline-справки, slim — в 3–4 раза меньше плюс точечная подгрузка.

Если заметили, что качество анализа просело (особенно на слабых моделях вроде Haiku или мелких OSS) — попробуйте перезапустить сервер с `RLM_STRATEGY_MODE=full`: вернётся прежний формат стратегии целиком inline, тул `rlm_help` исчезнет из манифеста. Подробности — **[docs/ENV_REFERENCE.md](docs/ENV_REFERENCE.md)** (раздел «Стратегия `rlm_start`»).
...
Пять MCP-инструментов (`rlm_projects`, `rlm_index`, `rlm_start` → `rlm_execute` → `rlm_end`), песочница с **55 хелперами** (45 BSL-специфичных + 8 стандартных + 2 LLM), сессионные кэши, таймауты, безопасность, пример построения графа вызовов на крупной конфигурации (23K+ файлов) — **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)**
