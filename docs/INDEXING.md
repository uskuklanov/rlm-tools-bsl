...
Индекс хранится в SQLite-базе `bsl_index.db` и содержит 27 таблиц (4 основные + 17 метаданных + 1 навигационная + 4 reference-таблицы v12 + 1 code-usages-таблица v13) + виртуальную FTS5-таблицу для полнотекстового поиска:
...
| key                     | Описание                                                | Пример                             |
| ----------------------- | ------------------------------------------------------- | ---------------------------------- |
| `version`               | Версия схемы                                            | `13`                               |
| `builder_version`       | Версия построителя                                      | `13`                               |
...
- `manager` — обращение к менеджеру коллекции `Документы.X` / `Documents.X` (ищется в коде; строки и комментарии сняты через `_strip_code_line`);
